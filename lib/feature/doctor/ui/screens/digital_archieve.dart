...
class DigitalArchivePage extends StatelessWidget {
    ...
    return BlocBuilder<DoctorDataCubit, DoctorDataState>(
      builder: (context, state) {
        ...
        final List<Map<String, dynamic>> archivedFiles = 
            ...
            ? List<Map<String, dynamic>>.from(doctor.toMap()['digital_archive'] ?? []) 
            : [];
            ...
            appBar: AppBar(
              ...
              title: Row(
                children: [
                  ...
                  SizedBox(width: 12.w),
                  Text('archive.title'.tr(), style: textTheme.titleMedium?.copyWith(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16.sp)),
                ],
              ),
              bottom: TabBar(
                    ...
                    child: archivedFiles.isEmpty
                        ? Center(
                            child: Column(
                              ...
                              children: [
                                Icon(Icons.folder_open, size: 60.sp, color: Colors.grey.shade400),
                                SizedBox(height: 10.h),
                                Text("archive.no_files".tr(), style: TextStyle(color: Colors.grey.shade600, fontSize: 14.sp)),
                              ],
                            ),
                          )
                        : GridView.builder(
  ...
  // دالة مساعدة لاستخراج نوع الملف من الرابط
  ...
  Widget _buildAppBarProfile(ColorScheme colorScheme, String? imageUrl) {
    return Container(
      ...
      child: CircleAvatar(
        ...
        child: (imageUrl == null || imageUrl.isEmpty) ? Icon(Icons.person, color: Colors.white, size: 18.sp) : null,
      ),
    );
  }
  ...
  Widget _buildSearchAndSortBar(ColorScheme colorScheme) {
    return Row(
      children: [
        ...
        SizedBox(width: 10.w),
        Text("archive.sort_by_date".tr(), style: TextStyle(color: colorScheme.primary, fontWeight: FontWeight.w600, fontSize: 13.sp)),
        const Spacer(),
        GestureDetector(
          onTap: () {},
          child: Container(
      ...
      child: Container(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Center(child: Icon(fileIcon, size: 65.sp, color: iconColor.withOpacity(0.8))),
              SizedBox(height: 12.h),
              Text(fileName, maxLines: 2, overflow: TextOverflow.ellipsis, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12.sp, color: colorScheme.primary)),
              SizedBox(height: 4.h),
              Text(fileType.toUpperCase(), style: TextStyle(color: Colors.grey, fontSize: 10.sp, fontWeight: FontWeight.bold), maxLines: 1),
              const Spacer(),
              Align(alignment: Alignment.bottomRight, child: Icon(Icons.download_rounded, size: 18.sp, color: colorScheme.secondary)),
            ],
          ),
        ),
      ),
